...
def feedback_train(student_model, teacher_model, dataloader, criterion, optimizer, scheduler, agent, epochs=10, is_decoder=True, beta=100):
    ...
    Loss_list = []
    with open(os.path.join(BASE_DIR, "values", PEFT_MODEL_NAME, "values.jsonl"), "a+") as f:
        json_obj = json.dumps({
            "id": 0,
            "T": T_list[-1],
            "Q": Q_list[-1],
...
def persd_train(student_model, teacher_model, dataloader, criterion, optimizer, scheduler, agent, epochs=10, is_decoder=True, beta=100):
    T_list = []
    T_list.append(criterion.get_T())
    Q_list = []
    Q_list.append(agent.get_q().item())
